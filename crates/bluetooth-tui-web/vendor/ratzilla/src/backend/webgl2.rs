...
pub struct WebGl2Backend {
    ...
    /// Performance measurement.
    // Locally patched: measure_begin/measure_end below no longer read
    // this - see their doc comments.
    #[allow(dead_code)]
...
impl WebGl2Backend {
    ...
    ///
    /// Locally patched: `Performance::mark`/`measure_with_start_mark`
    /// aren't available with just the "Performance" web-sys feature on
    /// current web-sys (0.3.8x+) - upstream hasn't picked up whatever
    /// feature split introduced that. Not worth chasing down for
    /// WebGl2Backend-only perf telemetry we (bluetooth-tui-web, which
    /// only ever uses DomBackend) never observe anyway.
    fn measure_begin(&self, _label: &str) {}

    /// Measures the end of a performance mark. See `measure_begin`.
    fn measure_end(&self, _label: &str) {}
